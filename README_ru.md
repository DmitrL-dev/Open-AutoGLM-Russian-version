...
[![Security](https://img.shields.io/badge/Security-Hardened-brightgreen.svg)](#исправления-безопасности)
...
flowchart TB
    ...
    subgraph Agent["🤖 Phone Agent Core"]
        ...
        PA[PhoneAgent]
        AC[AgentConfig]
        DS[Проверка устройства]
        VA[Валидатор]
    end
    
    subgraph Model["🧠 AI Модель"]
        MC[ModelClient]
        VLM[Vision-Language Model<br/>AutoGLM-Phone-9B]
    end
    
    subgraph Actions["⚡ Обработчик действий"]
        AH[ActionHandler]
        Parser[Безопасный парсер<br/>Без eval!]
    end
    
    subgraph ADB["📱 Слой ADB"]
        direction TB
        Conn[Менеджер подключений]
        Dev[Управление устройством]
        SS[Скриншоты]
        Input[Ввод текста]
    end
    
    subgraph Phone["📲 Android устройство"]
        Screen[Экран]
        Apps[Приложения]
    end
...
flowchart LR
    subgraph phone_agent["📦 phone_agent/"]
        ...
        subgraph core["Ядро"]
            agent[agent.py<br/>Класс PhoneAgent]
            init[__init__.py<br/>Экспорты пакета]
        end
        
        subgraph adb["adb/"]
            connection[connection.py<br/>USB/WiFi/Remote]
            device[device.py<br/>tap, swipe, back]
            screenshot[screenshot.py<br/>Захват экрана]
            input[input.py<br/>ADB Keyboard]
        end
        
        subgraph actions["actions/"]
            handler[handler.py<br/>Выполнение действий]
        end
        
        subgraph model["model/"]
            client[client.py<br/>OpenAI-совместимый API]
        end
        
        subgraph config["config/"]
            apps[apps.py<br/>Маппинг приложений]
            prompts_en[prompts_en.py]
            prompts_ru[prompts_ru.py]
            i18n[i18n.py<br/>Переводы]
        end
        
        subgraph utils["Утилиты (НОВОЕ)"]
            utils_py[utils.py<br/>Retry и логирование]
            device_state[device_state.py<br/>Проверки устройства]
            validation[validation.py<br/>Валидация ответов]
        end
    end
